...
parameterdf=pd.read_excel(codespace+"/"+"Anhang1_Parameter_Waldstandorte_BE_20220908.xlsx", dtype="str", engine='openpyxl')
parameterdf.columns
...
parameterdf=parameterdf.rename(columns={"Subregionen (nur in diesen vorkommend)":"Subregionen"})
parameterdf=parameterdf.rename(columns={'Arrondieren_korrigierter Vorschlag BH (alle einzelnen Pixel von "x" weglassen)':'arrondieren'})
parameterdf=parameterdf.rename(columns={'SW Kristallines Gebiet (wenn "x", dann soll diese Einheit modelliert werden können)':'kristallin'})
parameterdf=parameterdf.rename(columns={'Kristallin: nur Einheiten mit "x" oder "s" auf Fels modellieren (x = doppelt gewichten, s = nur halb gewichten)':"FelsKristallin"})
parameterdf.columns
...
convertarrtotif(outarr, outdir+"/"+"bestandortstypen.tif", 3, referenceraster, NODATA_value)
...
while i<nrows-1:
    ...
    while j<ncols-1:
        ...
        j+=1
    i+=1
convertarrtotif(outarrarrondiert, outdir+"/"+"bestandortstypenarrondiert.tif", 3, referenceraster, NODATA_value)
convertarrtotif(arrondiertarr, outdir+"/"+"bearrondiert.tif", indatatypeint, referenceraster, NODATA_value)
...
#aggregate areas
bestandortstypengdfmergeclip=gpd.read_file(outdir+"/"+"bestandortstypenjoinedclipwald.shp")
...
areastatisticsJura.to_excel(outdir+"/areastatisticsJura.xlsx")
areastatisticsMittelland.to_excel(outdir+"/areastatisticsMittelland.xlsx")
areastatisticsAlpen.to_excel(outdir+"/areastatisticsAlpen.xlsx")
#areastatistics.to_excel(outdir+"/areastatistics.xlsx")

#arrondiertes file
bestandortstypenarrondiertgdf=gpd.read_file(outdir+"/"+"bestandortstypenarrondiert.shp")
...
bestandortstypenarrondiertgdfmerge.to_file(outdir+"/bestandortstypenarrondiertjoined.shp")
print("exported joined  shapefile")